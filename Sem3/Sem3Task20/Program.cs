...
//1) Метод общения с пользователем Read(), по очереди должны у него запросить 
//х1, х2, y1, y2
//2) Вычисление, т.к корень поэтому будет не целое число , значит тип 
// не int ,а double Colculite(int х1, int y1, int х2, int y2)
//создаем переменную double res = 0
//формула return Math.Sqrt((х2 - х1) * (х2 - х1) + (y2 - y1) * (y2 - y1))
//3) Метод это результат 
