...
Dictionary<int, int> GenerateDicPoli()//метод который создаст словарь
//Собираем словарь полиндрома, возвращаем тот который обьявили return numberP
{
    Dictionary<int, int> palidrom = new Dictionary<int, int>();

    for (int i = 10; i < 100; i++)// нужно его заполнить, берем  for т.к. 
    //многие его любят. 
    
   
    {
        int polik = (i) * 100 + (i % 10) * 10 + i / 10;//в словарь кладем значения
        palidrom.Add(polik,0);// для ускорения пишем значение polik
